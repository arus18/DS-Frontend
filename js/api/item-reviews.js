...
function addReview() {
  ...
  // Get the buyer ID from some source (e.g., user authentication)
  const buyerId = "123";

  // Make a fetch request to save the item review
  fetch(`${BASE_URL}/itemsreviews`, {
    method: 'POST',
    body: JSON.stringify({item_id: itemId, buyer_id: buyerId ,review: reviewText}),
    headers: {
  ...
  function fetchItemReviews(itemId) {
    ...
    fetch(`${BASE_URL}/itemsreviews/${itemId}`)
      .then(response => response.json())
      .then(reviews => {
        console.log(itemId);
        ...
        reviews.forEach(review => {
          ...
          reviewDiv.textContent = review.review;
